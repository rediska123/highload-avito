...
Полнотекстовый поиск в Manticore search осуществляется с помощью инвертированного индекса - структуры данных, в которой для каждого уникального слова хранится список записей, в которых встречается это слово. 

Перед сопоставлением с индексом поисковый запрос токенизируется - разбивается на отдельные термы. При этом используются те же правила, что применялись при индексации строк. Это обеспечивает согласованность между формой слов в индексе и в запросе
  
При индексации и токенизации Manticore применяет языковую нормализацию:
- Исключаются стоп-слова из индекса, так как они не несут смысловой нагрузки и лишь увеличивают размер индекса
- Слова приводятся к их основной форме (Стемминг), что позволяет находить записи по разным грамматическим формам одного слова

По умолчанию поиск выполняется по всем проиндексированным текстовым полям, однако при необходимости область поиска можно ограничить определенным набором полей или задать условия с помощью операторов

После выполнения полнотекстового поиска выполняется фильтрация по атрибутам. Фильтрация опирается на колоночное хранение данных и специализированные индексы по атрибутам, что обеспечивает высокую производительность даже при большом объёме данных.

Manticore предлагает несколько встроенных ранкеров, наиболее подходящий "proximity_bm25". 
Он сочетает в себе статистическую оценку BM25 и lcs - длину самой длинной подпоследовательности ключевых слов из запроса, встречающейся в записи в том же порядке. Эта мера близости (lcs) важна для точных запросов. 

Функция ранжирования BM25 (Best Match 25) оценивает релевантность на основе трёх ключевых факторов:   
- Частота слова в тексте объявления  
- Редкость слова во всем словаре - если редко встречается значит вносит больший вклад
- Длина текста объяления - короткие объявления получают преимущество, чтобы избежать завышения релевантности у избыточно длинных текстов

Итоговый вес вычисляется по формуле

`sum(lcs * user_weight) * 1000 + bm25`

где user_weight - вес назначаемый каждому текстовому полю при индексации, например заголовок может иметь больший вес чем описание. Эти веса задаются в настройках индекса через параметр field_weights

После полнотекстового поиска и фильтрации результаты упорядочиваются. По умолчанию - по убыванию веса релевантности, вычисленного ранкером; при явном указании - по значению атрибута, например по цене или дате. Однако, если задана сортировка, то чтобы объявления с одинаковым значением атрибута, по которому происходит сортировка, были расположены в соответствии с релевантностью, нужно включить опцию track_scores=true в запрос

Для исправления возможных опечаток в Manticore search используется механизм автокоррекции. Он позволяет находить похожие слова из проиндексированного словаря, даже если пользовательский запрос содержит неточности. Используется расстояние Левенштейна, которое показывает сколько нужно сделать операций вставки, удаления или замены символов для превращения одного слова в другое. Если вычисленное расстояние меньше заданного порога, то слово является кандидатом на исправление. Дополнительно проверяется переключения раскладки, система переведет введенное слово в другую раскладку и повторит сравнение. Полученные слова сортируются по расстоянию и частоте слова в индексе

## Технологии

| Технология | Область применения | Мотивационная часть |
|--------------------------|--------------------|--------------------------------------------------------|
| TypeScript | Frontend | Строгая типизация, расширение js |
| Svelte | Frontend | Работает без vdom, компилирует в чистый js, меньше весит |
| PostgreSQL | БД | Поддержка сложных типов, высокая производительность для сложных запросов и больших баз, надежность |
| Minio | БД | Совместим с Amazon S3, шардирование из коробки, безопасность |
| Manticore | БД | Полноценный поиск по тексту, поддерживает json. Скорость выше чем у elasticsearch |
| Redis | БД | Хранит данные в памяти, можно установить время жизни сессии |
| Kafka | БД | Гарантия доставки, репликация, отказоустойчивость. Выдерживает большие нагрузки |
| ClickHouse | БД | Эффективно сжимает данные, оптимизирован под аналитические запросы |
| Docker | DEV | Создание изолированных сред. Меньше весит и ест ресурсов чем виртуалка. Есть своя экосистема, подробная документация |
| Kubernetes | DEV | Автоматизация развертывания. Сам перезапускает отказавшие контейнеры, автоматическое масштабирование, балансировка  |
| Gitlab | DEV | Git сервер с поддержкой issues, ci cd, код ревью |
| Go (Golang) | Backend | Один бинарник, экосистема для бд, строгая типизация, хорошо параллелиться|
| Vault | Backend | Хранилище для секретов |
| Prometheus | Backend | Сбор метрик сервисов |
| Grafana | Backend | Визуализация метрик, алёрты, уведомления |
| Loki | Backend | Собирает логи и эффективно их хранит. От создателей grafana |
| Nginx | Прокси | Высокая производительность, надёжность, SSL |
| PgBouncer | Прокси | Балансировки нагрузки для postgreSQL |

## Обеспечение надёжности

| Технология | Обеспечение надежности |
|------------------------------|--------------------------|
| Manticore | Резервирование через встроенный механизм репликации на базе библиотеки Galera. Galera использует технологию синхронной многомастеровской репликации |
| ClickHouse | Резервирование с помощью движка ReplicatedMergeTree, который дублирует все вставляемые данные в реплику через координацию с помощью ZooKeeper. В ClickHouse Cloud репликация обрабатывается автоматически |
| PostgreSQL | Шардирование через Shardman. Потоковая репликация для копий. При потоковой репликации мастер сервер передает wal-логи (из журнала транзакций) на standby-сервер, чтобы тот выполнил те же изменения для поддержания синхронности данных|
| MinIO | Храним картинки в нескольких ДЦ. Встроенный алгоритм стирающего кода, где файл (картинка) разбивается на кусочки и к ним приписывается избыточная информация, которая помогает восстановить файл при потере части файла |
| Redis | Периодические делает снимки всего набора данных с помощью механизма RDB. Ведет логи как модифицируются данные. Есть встроенная репликация |
| Микросервисы | Развернуты в Kubernetes который сам следит за их работой. Если сервис упал, то Kubernetes поднимет его заново, но это займет какое-то время. Для решения этой проблемы можем развернуть 2 экземпляра сервиса, чтобы в случае перезагрузки резервный мог взять нагрузку на себя. |
| Метрики и логи | Централизованный сбор метрик и логов с помощью Prometheus и Loki. Для визуализации данных и отправки уведомлений в случае сбоев работы используется Grafana |

## Схема проекта

<img width="4084" height="5204" alt="изображение" src="https://github.com/user-attachments/assets/2400636a-6f55-4054-8d88-fcc805fba773" />
